...
class Settings(BaseSettings):
    ...
    def vector_db_config(self) -> dict:
        return {"provider": self.VECTOR_DB_PROVIDER, "pinecone": {"api_key": self.PINECONE_API_KEY, "index_name": self.PINECONE_INDEX_NAME, "host": self.PINECONE_HOST}, "qdrant": {"host": self.QDRANT_HOST, "port": self.QDRANT_PORT, "collection": self.QDRANT_COLLECTION}}

    @property
    def embedding_config(self) -> dict:
        return {"provider": self.EMBEDDING_PROVIDER, "nvidia": {"api_key": self.NV_API_KEY, "base_url": self.NV_API_BASE_URL, "model": self.NV_EMBED_MODEL}, "openai": {"api_key": self.OPENAI_API_KEY, "model": self.OPENAI_EMBED_MODEL}}

    @property
    def llm_config(self) -> dict:
        return {"provider": self.LLM_PROVIDER, "nvidia": {"api_key": self.NV_CHAT_API_KEY or self.NV_API_KEY, "base_url": self.NV_API_BASE_URL, "model": self.NV_CHAT_MODEL}, "openai": {"api_key": self.OPENAI_API_KEY, "model": self.OPENAI_CHAT_MODEL}}
